...
public class DirectionsParser {
    ...
    public List<LatLng> parseOverviewPolyline(JSONObject jObject) {
        List<LatLng> path = new ArrayList<>();
        try {
            JSONArray jRoutes = jObject.getJSONArray("routes");
            String polyline = (String) ((JSONObject)((JSONObject) jRoutes.get(0)).get("overview_polyline")).get("points");
                ...
                for (int j = 0; j < jLegs.length(); j++) {
                    ...
                    for (int k = 0; k < jSteps.length(); k++) {
                        ...
                        for (int l = 0; l < list.size(); l++) {
                            ...
                            map.put("lat", Double.toString(list.get(1).latitude));
                            map.put("lon", Double.toString(list.get(1).longitude));
                            path.add(map);
                        }
                    }
                    routes.add(path);
